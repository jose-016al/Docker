...
formCancion.addEventListener('change', (e) => {
    e.preventDefault();
    player.src = URL.createObjectURL(cancion.files[contador]);
});
...
function reproduccion() {
    console.log(cancion.files.length);
    ...
    if (player.paused) {
        ...
        name.innerHTML = cancion.files[contador].name;
        duracion.innerHTML = formatearDuracion(player.duration);
        player.addEventListener("ended", () => {
            contador++;
            if (contador < cancion.files.length) {
                player.src = URL.createObjectURL(cancion.files[contador]);
                reproduccion();
            }
        }, false);
...
button.addEventListener('click', reproduccion)
